...
int main()
{
    ...
    std::vector<std::set<int>> item_interact(ITEM_NUMBER);
    for (int i = 0; i < USER_NUMBER; ++i)
    {
        ...
        while (data_input.peek() != '\n' && data_input.peek() != '\r')
        {
            ...
            item_interact[item_id].insert(user_id);
            // if (item_rating < 0) 
            //     item_rating = NO_RATING_PLACEHOLDER;
            // else 
            //     item_rating = RATING_PLACEHOLDER;
            input_triplets.emplace_back(item_id, user_id, item_rating);
    ...
    std::vector<std::vector<int>> similarity(USER_NUMBER);
    for (auto & row : similarity) row.resize(USER_NUMBER);
    ...
    // int predict_rating_max = 0;

    auto userTask = [&item_interact, &similarity, &ratings, &output_temp_files](int id, int begin, int end)
    {
        for (int i = begin; i < end; ++i)
        {
            ...
            {
                if (item_interact[j].find(i) != item_interact[j].end())
                    continue;

                std::vector<int> most_similar_users;
                for (int l : item_interact[j])
                {
                    int sim = similarity[l][i];
                    most_similar_users.emplace_back(sim);
                }
                // VERBOSE("sim > 0 items: " << most_similar_users.size());
                // std::sort(most_similar_users.rbegin(), most_similar_users.rend());
                // if (most_similar_users.size() > k)
                //     most_similar_users.resize(k);
                ...
                // if (predict_rating > predict_rating_max) predict_rating_max = predict_rating;
    ...
    // VERBOSE("predict_rating_max " << predict_rating_max);
