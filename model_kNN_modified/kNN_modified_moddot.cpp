...
auto mask(const SparseMatrix<double> &lhs, const SparseMatrix<double> &rhs)
{
    struct DoubleAndReserveLhs
    {
        double operator()(double lhs, double rhs) const
        {
            ...
            else if (lhs == 0.5 && rhs == 1 || rhs == 0.5 && lhs == 1 || rhs == 0.5 && lhs == 0.5) return 0.5;
            return 0.0;
        };
    };
...
int main()
{
    ...
    std::vector<std::vector<double>> similarity(ITEM_NUMBER);
    ...
    auto simTask = [&ratings, &similarity](int id, int begin, int end)
    {
        for (int j = begin; j < end; ++j)
        {
            ...
            {
                // if (l == j) continue;
                double sim = mask(ratings.col(j), ratings.col(l)).sum();
